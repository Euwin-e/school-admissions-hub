...
export default function ToValidateList() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Candidatures à valider</h1>
        <p className="text-gray-600">
          {filteredApplications.length} candidature{filteredApplications.length > 1 ? 's' : ''} en attente de votre décision
        </p>
      </div>
          ...
          {filteredApplications.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                Toutes les candidatures ont été traitées ou aucune n'a été transmise pour validation.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <Badge className="bg-bourbon">À valider</Badge>
                      </div>
                      ...
                      {application.agentMissingDocument && (
                        <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                          <p className="text-sm text-amber-800">
                            <span className="font-medium">Note agent:</span> {application.agentMissingDocument}
                          </p>
                          {application.agentReviewedBy && (
                            <p className="text-xs text-amber-600 mt-1">
                              Traité par l'agent ID: {application.agentReviewedBy}
                            </p>
                          )}
                        </div>
                      )}
